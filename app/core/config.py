...
class Settings(BaseSettings):
    ...
    #
    # 이 단계는 추론이 아니라 구조화된 추출 작업이라 사고를 낮출 여지가 크다
    # (계측: 분석 한 번에 19.8초 = 전체의 24%). 그래서 none 으로 뒀었는데,
    # **2026-09-08 파드 검증에서 그게 틀렸다는 것이 드러났다.**
    #
    # 동시 20명 프롬프트 퀴즈에서 5건이 422(주제 파악 실패)로 떨어졌다.
    # 실패 주제는 코스피/익절매/포트폴리오/영업이익/당기순이익 - 전부 초급
    # 카탈로그에 **문자 그대로는 없는** 것들이다. 카탈로그에 없는 주제를
    # 비슷한 카테고리로 옮기는 일에는 사고가 필요한데 none 은 그걸 못 한다.
    #
    # 같은 10개 주제 x 3회 = 30건으로 강도만 바꿔 재봤다 (로컬 OpenRouter):
    #
    #   none     13/30   <- 절벽
    #   minimal  30/30
    #   low      30/30
    #   medium   30/30
    #
    # 지연은 OpenRouter 변동폭(같은 작업이 6초와 48초)에 묻혀 minimal/low/medium
    # 사이에 차이를 볼 수 없었다. 그래서 절벽 바로 위(minimal)가 아니라 한 칸 위인
    # low 를 쓴다 - 이 측정은 OpenRouter 라 파드 vLLM 과 미세하게 다를 수 있고,
    # 여유가 없는 선택을 할 이유도 없다(지연 차이가 관측되지 않았으므로).
    #
    # 안전망(_validate_plan_item 이 카탈로그에 없는 코드를 거르고
    # _enforce_question_type 이 유형을 바로잡는 것)은 **잘못 뽑힌 값**을 막을 뿐,
    # 아무것도 못 뽑는 실패는 막지 못한다. 이번에 그 구분을 배웠다.
    #
    # 빈 문자열이면 LLM_REASONING_EFFORT 를 그대로 쓴다.
    ANALYZE_REASONING_EFFORT: str = "low"
