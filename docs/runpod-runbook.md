...
> ⚠️ **다음 배포에서 반드시 한 번 거쳐야 하는 단계가 있다 — §6-1 파드 Qdrant 공시 재적재.**
> 2026-09-15 에 고친 공시 단위 환산·중복 결함은 **코드만 고쳐서는 이미 적재된 문서에
> 반영되지 않는다.** 로컬은 정리했지만 파드의 Qdrant 는 Network Volume 에 따로 있어
> 파드를 지워도 오염된 공시가 그대로 남아 있다.
...
**"서버와 같은 모델"이 아니다 — 이름만 같다.** OpenRouter 의 `qwen/qwen3.8-27b` 는
모델 이름 하나 뒤에 프로바이더를 여럿 둔 라우터이고, 양자화가 제각각이다.

    2026-09-15 실측: 프로바이더 16곳
      bf16     DeepInfra
      fp4      Darkbloom
      fp8      Reka / Parasail / AkashML / CoreWeave / Chutes / Ionstream / …
      unknown  DekaLLM / Phala / Novita / Alibaba / Cloudflare

고정하지 않으면 요청마다 다른 곳으로 간다 — **같은 요청 10회에 6개 프로바이더가
응답했고 그중 fp4 가 3회**였다. 파드는 `Qwen/Qwen3.8-27B-FP8` 하나뿐이므로,
로컬에서 잰 수치가 파드를 대표하지 못할 뿐 아니라 **로컬끼리도 재현되지 않는다.**

그래서 `LLM_OPENROUTER_QUANTIZATIONS=fp8` 로 고정한다(`.env` 기본값).
`LLM_BASE_URL` 이 openrouter.ai 일 때만 요청에 실리고 파드로는 나가지 않는다.

**고정의 효과는 "파드와 같은 출력"이 아니라 "매번 같은 조건"이다.** 정밀도를 맞춰도
KV 캐시 dtype·컨텍스트 길이·샘플링 구현이 달라 출력은 여전히 다를 수 있다.
...
| **양자화** | ⚠️ 고정해야 같아짐 | 고정 안 하면 요청마다 bf16/fp8/fp4 가 섞인다 |
| **KV 캐시 dtype** | ❌ 제어 불가 | 파드는 `--kv-cache-dtype fp8`, 프로바이더는 알 수 없다 |
| **컨텍스트 상한** | ❌ 다름 | 파드 16,384 / 프로바이더 262K~1M — 로컬에서만 안 걸린다 |
...
| LLM 양자화 | OpenRouter fp8 고정 | FP8 가중치 + fp8 KV 캐시 | **중간** — 가중치는 맞췄지만 KV 캐시·샘플링 구현은 다르다 |

**그래서 로컬 수치를 어디까지 믿을 것인가.**

| 고친 것 | 파드 전이 |
|---|---|
| 검색·적재·파싱 등 결정론적 코드 | **그대로 간다.** LLM 이 개입하지 않는다 |
| 프롬프트 변경 | **방향은 가고 수치는 다르다.** 기준선도 개선폭도 파드에서 다시 재야 한다 |
| 지연·동시성·프록시 | **로컬에서 못 본다.** §9 함정 기록 참고 |
...
### 6-1. 파드 Qdrant 공시 재적재 — **다음 배포에서 한 번만**

2026-09-15 에 공시 적재 결함 두 가지를 고쳤다 (`stockmate-rag-audit.md` §3·§4).

- 단위 환산이 주식 수·원 단위 칸까지 곱했다 (`712,702,365주` → `약 712.70조원`,
  `시설자금(원) 43,140,750,000,000` → `약 43,140,750조원` = 100만 배)
- 저정보 공시가 같은 내용으로 최대 56건씩 중복 저장됐다

**코드 수정만으로는 이미 적재된 문서가 바뀌지 않는다.** `ingest_disclosures` 가 문서 id 로
중복을 걸러 재적재를 건너뛰기 때문이다. 로컬은 정리했지만 파드의 Qdrant 는
Network Volume(`/workspace/qdrant_storage`)에 따로 있고 **파드를 지워도 남으므로**
오염된 공시가 그대로 있다.

파드 웹 터미널에서 한 번 실행한다.

```bash
cd /app && python3 scripts/reingest_disclosures.py
```

**언제 실행하나 — 파드가 뜬 직후, §7 스모크 전에.** 적재 캐시(30분)는 프로세스 메모리에
있어서 기동 직후에는 비어 있다. 그 시점에 지우면 재시작 없이 다음 질의부터 바로잡힌
본문으로 다시 쌓인다. 서비스를 연 뒤에 지우면 캐시 때문에 최대 30분간 공시가 비어 보인다.

지운 뒤 확인:

```
삭제 완료: source=dart, 전체 종목 → N건 제거
```

뉴스·재무 문서는 건드리지 않는다. 한 번 돌리고 나면 이후 배포에서는 필요 없다.
